...
async def process_faq_ticket(prisma: Prisma, session, ticket):
    ...
    try:
        ...
        # もしticketのtitleが空、かつ最初の質問文が存在したら、質問文から要約を生成する
        if conversations:
            # 最初のconversationを取得する
            first_conversation = conversations[0]

            if ticket.title is None and first_conversation.text is not None:
                question = first_conversation.text
                ...
                async with session.post(
                    ...
                    if response.status == 200:
                        ...
                        # タイトル生成の結果をログに記録
                        await append_log_entry(prisma, conversation_id, "title_generate", {
                            "status": "success",
                            "title": title,
                        ...
                        print(f"[FAQ] チケット {ticket.id} のタイトルを「{title}」に更新しました")
                    else:
                        response_text = await response.text()
                        print(f"[FAQ] Dify API (TICKET_TITLE_GENERATOR) エラー: ステータスコード {response.status}")
                        print(f"[FAQ] レスポンス内容: {response_text}")
                        print(f"[FAQ] リクエスト内容 (質問): {question[:100]}...")
                        
                        # タイトル生成のエラーをログに記録
                        await append_log_entry(prisma, conversation_id, "title_generate", {
                            "status": "error",
                            "error_code": response.status,
                            "error_message": response_text[:500]  # 長すぎる場合は切り捨て
                        })

        # conversationsの内容を全てStrで連結し、latest_questionに格納する
        # format
        # <conversation>
        # text: .text
...
"""
        ...
        # 他の会話も含めて、全ての会話をother_conversationsに格納する
        other_conversations = await prisma.conversation.find_many(
            where={
                'accountId': ticket.accountId,
                'tenantId': ticket.tenantId,
                'ticketId': {
        ...
        other_conversations_str = ""
        for conversation in other_conversations:
            other_conversations_str += f"""
<conversation>
text: {conversation.text}
questionType: {conversation.questionType}
creator: {conversation.creator}
...
"""
        # ここのother_conversations_strを使うことで、他の会話も含めて、全ての会話を含めて、Difyに渡すことができる。
        # ただし、一旦精度低下を招く可能性もあるので、止めておく。
...
"""
        ...
        if original_length > MAX_CHARS:
            knowledgeDataStr = knowledgeDataStr[:MAX_CHARS]
            print(f"[FAQ] 警告: knowledgeDataの文字数が上限を超えています。元の長さ: {original_length}文字、15万文字に切り詰めて送信します。")
        ...
        if latest_question != "":
            ...
            async with session.post(
                ...
                if response.status == 200:
                    ...
                    print(f"[FAQ] DEBUG:ストリーミング応答開始: チケットID {ticket.id}")
                    ...
                    try:
                        ...
                        if "Chunk too big" in str(e):
                            ...
                            await append_log_entry(prisma, "error", error_data)
                    except Exception as e:
                    ...
                    result_code = final_outputs.get('resultCode', '')
